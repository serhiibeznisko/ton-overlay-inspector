...
class Node(Server):
    ...
    def __init__(
        ...
        self._lost_pings = 0
        self.shared_key = get_shared_key(transport.client.x25519_private.encode(), self.x25519_public.encode())
    ...
    async def connect(self):
        res = await self.transport.connect_to_peer(self)
        assign_number(self)
        return res
    ...
    async def ping(self):
        while True:
            try:
                ...
                self._lost_pings += 1
                # self.logger.warning(f'Lost ping {get_number(self)}, wait: {time.time() - start}, q: {self.transport.sock._packets.qsize()}')
    ...
    async def disconnect(self):
        num = get_number(self)
        # self.logger.warning(f'Disconnected {get_number(self)}')
        if self.connected:
...
class PeerAlreadyConnected(AdnlTransportError):
    pass
...
class AdnlTransport:
    ...
    async def send_message_outside_channel(self, data: dict, peer: Node) -> list:
        ...
        init_cipher = create_aes_ctr_sipher_from_key_n_data(peer.shared_key, checksum)
    ...
    async def start(self):
        ...
        sock = SocketProtocol(timeout=self.timeout)
    ...
    async def connect_to_peer(self, peer: Node) -> list:
        ...
        if peer.key_id in self.peers:
            raise PeerAlreadyConnected(f"Peer {peer.key_id.hex()} is already connected")
