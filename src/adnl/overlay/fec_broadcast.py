...
class BroadcastFec:
    ...
    async def distribute_part(self, seqno: int, invisible=None):
        if seqno not in self.parts:
            return
        data = self.parts.get(seqno)

        if invisible:
            return invisible.send_custom_message(data)
...
class BroadcastFecPart:
    ...
    async def run(self, invisible=None):
        if not invisible:
            try:
                self.run_checks()
        ...
        await self.distribute(invisible)

    async def distribute(self, invisible=None):
        await self.brcst.distribute_part(self.seqno, invisible)
...
async def create_fec_broadcast(overlay: OverlayTransport, data: bytes, flags: int, invisible=None):
    if len(data) > 1 << 27:
